...
def set_pcq_resnet(fused, pre):
    ...
    bn_momentum = nn.Parameter(torch.tensor(fused.arg_dict['bn_momentum']), requires_grad=False)
    # First layer
    ...
    fused.bn1.momentum = bn_momentum
    ...
    if block[0].downsample is not None:
        ...
        block[0].bn_down.momentum = bn_momentum
    for i in range(len(block)):
        ...
        block[i].bn1.momentum = bn_momentum
        block[i].bn2.momentum = bn_momentum
