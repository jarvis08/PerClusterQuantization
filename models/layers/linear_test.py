...
class QuantizedLinear(nn.Linear):
    ...
    def forward(self, x):
        if self.classifier != 3:
            ...
            torch.cuda.synchronize()
            print()
            print("Gemm mean time: {}".format(start.elapsed_time(end) / 20))

            print()
            # gemm_end = time()
            # print("Gemm mean time: {} sec".format((  gemm_end-gemm_start) / 20))

        if self.runtime_helper.batch_cluster is not None:
            if not self.check_time:
                return self.normal_pcq_totalsum(x.type(torch.cuda.IntTensor), sum_q1q2.type(torch.cuda.IntTensor))
            self.classifier = 1
            self.check_time = False
            x = x.type(torch.cuda.IntTensor)
            sum_q1q2 = sum_q1q2.type(torch.cuda.IntTensor)
            bc = self.runtime_helper.batch_cluster
            self._z1 = torch.index_select(self.z1, 0, bc)
            self._sum_a2 = torch.sum(self.weight, dim=1).view(1, -1).repeat(x.size(0), 1).mul(self._z1[:, None])
            self._nz1z2 = x.size(1) * self._z1 * self.z2
            ...
            self._z1 = torch.index_select(self.z1, 0, bc)
            self._z3 = torch.index_select(self.z3, 0, bc).reshape(bc.shape[0], 1)
            self._M0 = torch.index_select(self.M0, 0, bc).reshape(bc.shape[0], 1)
            self._shift = torch.index_select(self.shift, 0, bc).reshape(bc.shape[0], 1)
            self._bias = torch.index_select(self.quantized_bias, 0, bc)
            ...
            torch.cuda.synchronize()
            print()
            print("Ready Total-sum mean time: {}".format(start.elapsed_time(end) / 10))
            ...
            out = self.pcq_totalsum(x, sum_q1q2)
            out = self.pcq_totalsum(x, sum_q1q2)
            out = self.pcq_totalsum(x, sum_q1q2)
            out = self.pcq_totalsum(x, sum_q1q2)
            out = self.pcq_totalsum(x, sum_q1q2)
            ...
            torch.cuda.synchronize()
            print()
            print("PCQ Total-sum mean time: {}".format(start.elapsed_time(end) / 20))

            # totalend_end = time()
            # print()
            # print("PCQ Total-sum mean time: {} sec".format(( totalend_end -totalsum_start) / 20))
            # return self.pcq_totalsum(x.type(torch.cuda.IntTensor), sum_q1q2.type(torch.cuda.IntTensor))
            return out
        else:
            if not self.check_time:
                return self.normal_general_totalsum(x.type(torch.cuda.IntTensor), sum_q1q2.type(torch.cuda.IntTensor))
            self.classifier = 1
            self.check_time = False
            self._nz1z2 = x.size(1) * self.z1 * self.z2
            x = x.type(torch.cuda.IntTensor)
            sum_q1q2 = sum_q1q2.type(torch.cuda.IntTensor)
            self._sum_a2 = torch.sum(self.weight, dim=1).mul(self.z1)
            ...
            out = self.general_totalsum(x, sum_q1q2)
            out = self.general_totalsum(x, sum_q1q2)
            out = self.general_totalsum(x, sum_q1q2)
            out = self.general_totalsum(x, sum_q1q2)
            out = self.general_totalsum(x, sum_q1q2)
            ...
            torch.cuda.synchronize()
            print()
            print("QAT Total-sum mean time: {}".format(start.elapsed_time(end) / 20))

            # totalend_end = time()
            # print()
            # print("QAT Total-sum mean time: {} sec".format(( totalend_end -totalsum_start) / 20))
    ...
    def pcq_totalsum(self, x, sum_q1q2):
        if self.is_bias:
            sum_q1q2.add_(self._bias)
        sum_a1 = torch.sum(x, dim=1).mul(self.z2)

        sum_q1q2 = sum_q1q2.add(self._nz1z2[:, None])
        sum_q1q2 = sum_q1q2.sub(sum_a1[:, None])
        sum_q1q2 = sum_q1q2.sub(self._sum_a2)

        multiplied = multiply_M(sum_q1q2.type(torch.cuda.LongTensor), self._M0)
        total = shifting2d(multiplied, self._shift)
        total = total.add(self._z3)
    ...
    def general_totalsum(self, x, sum_q1q2):
        ...
        sum_q1q2 = sum_q1q2.add(self._nz1z2)
        sum_q1q2 = sum_q1q2.sub(sum_a1[:, None])
        sum_q1q2 = sum_q1q2.sub(self._sum_a2[None, :])
    ...
    def normal_pcq_totalsum(self, x, sum_q1q2):
        bc = self.runtime_helper.batch_cluster
    ...
    def normal_general_totalsum(self, x, sum_q1q2):
        if self.is_bias:
            sum_q1q2.add_(self.quantized_bias[0][None, :])
