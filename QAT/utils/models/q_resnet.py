...
class Q_ResNet20_Daq(nn.Module):
    """
        Quantized ResNet20 model for dataset CIFAR100, CIFAR10
    """
    def __init__(self, model, runtime_helper, resize_qbit):
        super().__init__()
        features = getattr(model, 'features')
        init_block = getattr(features, 'init_block')
        ...
        for stage_num in range(0, 3):
            ...
            for unit_num in range(0, self.channel[stage_num]):
                ...
                quant_unit.set_param(unit, runtime_helper, resize_qbit=self.resize_qbit)
                setattr(self, f'stage{stage_num + 1}.unit{unit_num + 1}', quant_unit)
...
class Q_ResBlockBn_Daq(nn.Module):
    ...
    def set_param(self, unit, runtime_helper, resize_qbit=False):
        self.resize_identity = unit.resize_identity
        self.resize_qbit = resize_qbit
    ...
    def forward(self, x, scaling_factor_int32=None):
        ...
        if self.resize_identity:
            if self.resize_qbit:
                ...
                scaling_factor_int32 = act_scaling_factor.clone() if act_scaling_factor is not None else None
...
class Q_ResBlockBn(nn.Module):
    ...
    def set_param(self, unit, resize_qbit=False):
        self.resize_identity = unit.resize_identity
        self.resize_qbit = resize_qbit
...
def q_resnet20(model, resize_qbit=True, runtime_helper=None):
    if runtime_helper is None:
        net = Q_ResNet20(model, resize_qbit)
    else:
        net = Q_ResNet20_Daq(model, runtime_helper, resize_qbit)
    return net
