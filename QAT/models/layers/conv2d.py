...
class QuantizedConv2d(nn.Conv2d):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, activation=None,
        ...
        bit, self.per_channel, self.symmetric, self.num_clusters, self.runtime_helper, self.default_batch = \
            itemgetter('bit', 'per_channel', 'symmetric', 'cluster',
                       'runtime_helper', 'val_batch')(arg_dict)
        self.w_bit = nn.Parameter(torch.tensor(
            bit, dtype=torch.int8), requires_grad=False)
        self.a_bit = nn.Parameter(torch.tensor(
            bit, dtype=torch.int8), requires_grad=False)
        self.is_bias = nn.Parameter(torch.tensor(
            False, dtype=torch.bool), requires_grad=False)
        self.quantized_bias = nn.Parameter(torch.zeros(
            (self.num_clusters, out_channels), dtype=torch.int32), requires_grad=False)
        self.sum_a2 = nn.Parameter(torch.zeros(
            (1, out_channels, 1, 1), dtype=torch.int32), requires_grad=False)
        self.sum_a1 = None
        ...
        self.s1 = nn.Parameter(torch.tensor(
            t_init, dtype=torch.float32), requires_grad=False)
        self.s3 = nn.Parameter(torch.tensor(
            t_init, dtype=torch.float32), requires_grad=False)
        self.z1 = nn.Parameter(torch.tensor(
            t_init, dtype=torch.int32), requires_grad=False)
        self.z3 = nn.Parameter(torch.tensor(
            t_init, dtype=torch.int32), requires_grad=False)

        self.is_shift_neg = nn.Parameter(torch.tensor(
            False, dtype=torch.bool), requires_grad=False)
        if self.per_channel:
            self.s2 = nn.Parameter(torch.zeros(
                out_channels, dtype=torch.float32), requires_grad=False)
            self.z2 = nn.Parameter(torch.zeros(
                out_channels, dtype=torch.int32), requires_grad=False)
            self.M0 = nn.Parameter(torch.zeros(
                (self.num_clusters, out_channels), dtype=torch.int32), requires_grad=False)
            self.shift = nn.Parameter(torch.zeros(
                (self.num_clusters, out_channels), dtype=torch.int32), requires_grad=False)
        else:
            self.s2 = nn.Parameter(torch.tensor(
                0, dtype=torch.float32), requires_grad=False)
            self.z2 = nn.Parameter(torch.tensor(
                0, dtype=torch.int32), requires_grad=False)
            self.M0 = nn.Parameter(torch.tensor(
                t_init, dtype=torch.int32), requires_grad=False)
    ...
    def _conv_impl(self, x):
        padded = x
        if self.padding[0] > 0:
            to_pad = (self.padding[0], self.padding[0],
                      self.padding[1], self.padding[1])

            if self.num_clusters == 1:
                padded = F.pad(x, to_pad, mode='constant',
                               value=self.z1.item())
            else:
                ...
                if self.a_bit == 4 or self.a_bit == 32:
                    ...
                    padded = F.pad(x, to_pad, mode='constant',
                                   value=self.z1[bc].item())

        out = F.conv2d(padded, self.weight, None, self.stride,
    ...
    def _pcq_subsum(self, x, sum_q1q2):
        ...
        if self.is_bias:
            sum_q1q2 = sum_q1q2.add(
                self.quantized_bias[bc][None, :, None, None])
        if not self.symmetric:
            nz1z2 = self._pcq_calc_sum_a1_and_nz1z2(x, sum_q1q2)

            if self.per_channel:
                sum_a1 = self.sum_a1[:batch_size][:, None, :, :].mul(
                    self.z2[None, :, None, None])
            else:
    ...
    def _pcq_calc_sum_a1_and_nz1z2(self, x, sum_q1q2):
        batch_size = x.size(0)
        bc = self.runtime_helper.qat_batch_cluster
        ...
        if self.sum_a1 is None or self.sum_a1.shape[0] != input_batch:
            self.sum_a1 = torch.zeros(
                (input_batch, 1, output_col, output_row), dtype=torch.int32, device='cuda')
        for o_col in range(output_col):
            for o_row in range(output_row):
                ...
                self.sum_a1[:batch_size, 0, o_col, o_row] = torch.sum(
                    x[:, :, col_st: col_end, row_st: row_end], (1, 2, 3))

        nz1z2 = input_ch * filter_col * filter_row * self.z1[bc] * self.z2
        if self.per_channel:
            nz1z2 = nz1z2[None, :, None, None]
        return nz1z2
    ...
    def _general_subsum(self, x, sum_q1q2):
        ...
        if self.is_bias:
            sum_q1q2 = sum_q1q2.add(
                self.quantized_bias[0][None, :, None, None])
        ...
        if not self.symmetric:
            ...
            if self.sum_a1 is None or self.sum_a1.shape[0] != input_batch:
                self.sum_a1 = torch.zeros(
                    (input_batch, output_col, output_row), dtype=torch.int32, device='cuda')
            for o_col in range(output_col):
                for o_row in range(output_row):
                    col_st, col_end = o_col * stride, o_col * stride + filter_col
                    row_st, row_end = o_row * stride, o_row * stride + filter_row
                    self.sum_a1[:batch_size, o_col, o_row] = torch.sum(
                        x[:, :, col_st: col_end, row_st: row_end], (1, 2, 3))
...
class PCQConv2d(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, groups=1, dilation=1, bias=False,
        ...
        self.w_bit = torch.nn.Parameter(torch.tensor(
            w_bit, dtype=torch.int8), requires_grad=False)
        self.a_bit = torch.nn.Parameter(torch.tensor(
            a_bit, dtype=torch.int8), requires_grad=False)

        self.act_range = nn.Parameter(torch.zeros(
            (self.num_clusters, 2)), requires_grad=False)
        self.apply_ema = nn.Parameter(torch.zeros(
            self.num_clusters, dtype=torch.bool), requires_grad=False)
    ...
    def _fake_quantize_weight(self, x):
        ...
        if self.per_channel:
            ...
            s, z = calc_qparams(w.min(), w.max(), self.w_bit,
                                symmetric=self.symmetric)
            w = fake_quantize(self.conv.weight, s, z, self.w_bit,
                              symmetric=self.symmetric, use_ste=self.use_ste)

        out = F.conv2d(x, w, self.conv.bias, self.conv.stride,
    ...
    def _update_activation_range(self, x):
        ...
        if self.apply_ema[cluster]:
            self.act_range[cluster][0] = self.act_range[cluster][0] * \
                self.smooth + _min * (1 - self.smooth)
            self.act_range[cluster][1] = self.act_range[cluster][1] * \
                self.smooth + _max * (1 - self.smooth)
        else:
    ...
    def _fake_quantize_activation(self, x, external_range=None):
        ...
        if external_range is not None:
            s, z = calc_qparams(
                external_range[cluster][0], external_range[cluster][1], self.a_bit, zero)
        else:
            s, z = calc_qparams(
                self.act_range[cluster][0], self.act_range[cluster][1], self.a_bit, zero)
    ...
    def set_qparams(self, s1, z1, s_external=None, z_external=None):
        ...
        if s_external is not None:
            ...
            self.s3, self.z3 = calc_qparams_per_cluster(
                self.act_range, self.a_bit, zero)

        if self.per_channel:
            self.M0 = torch.zeros(
                (self.num_clusters, self.out_channels), dtype=torch.int32)
            ...
            for cluster in range(self.num_clusters):
                ...
                for channel in range(self.out_channels):
                    self.M0[cluster][channel], self.shift[cluster][channel] = quantize_M(
                        m_per_channel[channel])
        else:
...
class FusedConv2d(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, dilation=1, groups=1, bias=False,
        ...
        self.apply_ema = nn.Parameter(torch.tensor(
            0, dtype=torch.bool), requires_grad=False)
    ...
    def _fake_quantize_activation(self, x, external_range=None):
        if external_range is not None:
            s, z = calc_qparams(
                external_range[0], external_range[1], self.a_bit)
        else:
            s, z = calc_qparams(
                self.act_range[0], self.act_range[1], self.a_bit)
        return fake_quantize(x, s, z, self.a_bit, use_ste=self.use_ste)
    ...
    def _update_activation_range(self, x):
        ...
        if self.apply_ema:
            self.act_range[0] = self.act_range[0] * \
                self.smooth + _min * (1 - self.smooth)
            self.act_range[1] = self.act_range[1] * \
                self.smooth + _max * (1 - self.smooth)
        else:
    ...
    def set_qparams(self, s1, z1, s_external=None, z_external=None):
        ...
        if s_external is not None:
            ...
            self.s3, self.z3 = calc_qparams(
                self.act_range[0], self.act_range[1], self.a_bit)

        if self.per_channel:
            self.M0 = torch.zeros((1, self.out_channels), dtype=torch.int32)
            self.shift = torch.zeros((1, self.out_channels), dtype=torch.int32)
            m_per_channel = self.s1.type(
                torch.double) * self.s2.type(torch.double) / self.s3.type(torch.double)
            for channel in range(self.out_channels):
                self.M0[0][channel], self.shift[0][channel] = quantize_M(
                    m_per_channel[channel])
        else:
            self.M0, self.shift = quantize_M(self.s1.type(
                torch.double) * self.s2.type(torch.double) / self.s3.type(torch.double))
        return self.s3, self.z3
