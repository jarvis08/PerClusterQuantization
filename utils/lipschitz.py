...
class Simple224Network(torch.nn.Module):
    def __init__(self):
        ...
        # self.conv1 = torch.nn.Conv2d(3, 16, kernel_size=7, stride=2, padding=3, bias=False)
        # self.conv2 = torch.nn.Conv2d(16, 16, kernel_size=3, stride=1, padding=1, bias=False)
        # self.fc = torch.nn.Linear(200704, 1000, bias=False)
        self.conv1 = torch.nn.Conv2d(3, 16, kernel_size=7, stride=2, padding=3, bias=True)
        self.conv2 = torch.nn.Conv2d(16, 16, kernel_size=3, stride=1, padding=1, bias=True)
        self.fc = torch.nn.Linear(200704, 1000, bias=True)
        self.relu = torch.nn.ReLU(inplace=True)
...
def qat_lipschitz_with_zero(model, clustering_model, loader, input_shape, x_metric, y_metric):
    model.eval()
...
def check_lipschitz(args, tools):
    ...
    if args.dataset != 'imagenet':
        ...
        input_shape = (args.val_batch, 3, 32, 32)
    else:
        model = Simple224Network()
        input_shape = (args.val_batch, 3, 224, 224)
    model.cuda()
